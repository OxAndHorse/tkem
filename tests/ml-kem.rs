...
fn test_invalid_modulus(p: &str) {
    ...
    for line in reader.lines() {
        ...
        match p {
            ...
            "512" => assert!(!libcrux_ml_tkem::mlkem512::validate_public_key(
                &pk.try_into().unwrap()
            )),
            #[cfg(feature = "mlkem768")]
            "768" => assert!(!libcrux_ml_tkem::mlkem768::validate_public_key(
                &pk.try_into().unwrap()
            )),
            #[cfg(feature = "mlkem1024")]
            "1024" => assert!(!libcrux_ml_tkem::mlkem1024::validate_public_key(
                &pk.try_into().unwrap()
            )),
            _ => unreachable!(),
        };
...
fn test_invalid_dk(p: &str) {
    ...
    while let (Some(dk_string), Some(ct_string)) = (lines.next(), lines.next()) {
        ...
        match p {
            #[cfg(feature = "mlkem512")]
            "512" => assert!(!libcrux_ml_tkem::mlkem512::validate_private_key(
                &dk.try_into().unwrap(),
                &ct.try_into().unwrap(),
            )),
            #[cfg(feature = "mlkem768")]
            "768" => assert!(!libcrux_ml_tkem::mlkem768::validate_private_key(
                &dk.try_into().unwrap(),
                &ct.try_into().unwrap(),
            )),
            #[cfg(feature = "mlkem1024")]
            "1024" => assert!(!libcrux_ml_tkem::mlkem1024::validate_private_key(
                &dk.try_into().unwrap(),
                &ct.try_into().unwrap(),
            )),
            _ => unreachable!(),
